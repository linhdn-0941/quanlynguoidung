...
class AdminController
{
    ...
    {
        $query = '
            SELECT nguoidung.id, nguoidung.username, nguoidung.hoten,
            nguoidung.ngaysinh, nguoidung.gioitinh, vaitro.vaitro 
    ...
    public function validation($user, $create = true)
    {
        ...
        if (!preg_match($patternPassword, $user['confirm_password'])) {
            $error = 'Confirm password phải có ký tự viết hoa, viết thường, 
                    chữ số và lớn hơn 6 kí tự và nhỏ hơn 255  kí tự';
            array_push($errors, $error);
        }
